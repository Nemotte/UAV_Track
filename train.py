...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", type=str, default="sirstaug", help="choose datasets"
    )
    ...
    parser.add_argument(
        "--save-iter-step", type=int, default=20, help="save model per step iters"
    )

    #
    # Net parameters
    #
    parser.add_argument("--net-name", type=str, default="agpcnet", help="net name: fcn")
...
class Trainer(object):
        ...
        for i, (data, labels) in enumerate(tbar):
            ...
            if (self.iter_num % args.save_iter_step) == 0:
                ...
                self.writer.add_scalar(
                    "Losses/train loss", np.mean(losses), self.iter_num
                )
                self.writer.add_scalar(
                    "Learning rate/",
            ...
            tbar.set_description(
                "Epoch:%3d, lr:%f, train loss:%f, eval loss:%f, miou:%f/%f, fmeasure:%f/%f"
                % (
                    epoch,
                    trainer.optimizer.param_groups[0]["lr"],
                    np.mean(losses),
    ...
    def validation(self):
        ...
        miou, prec, recall, fmeasure = self.metric.get()
        pkl_name = "Iter-%5d_mIoU-%.4f_fmeasure-%.4f.pkl" % (
            self.iter_num,
            miou,
            fmeasure,
        )
